...
width, height = 600, 600 # Size of framebuffer
num_triangles = 100 # Number of random colored triangles to be generated
...
class TriangleRasterizer:
    def __init__(self, n):
        self.n = n
        self.A = ti.Vector.field(2, dtype=ti.f32, shape=n)
        self.B = ti.Vector.field(2, dtype=ti.f32, shape=n)
        self.C = ti.Vector.field(2, dtype=ti.f32, shape=n)
        self.c0 = ti.Vector.field(3, dtype=ti.f32, shape=n)
        self.c1 = ti.Vector.field(3, dtype=ti.f32, shape=n)
        self.c2 = ti.Vector.field(3, dtype=ti.f32, shape=n)
    ...
    def rasterize(self):
        for i, j in pixels:
            for k in range(self.block_num_triangles[i // tile_size, j // tile_size]):
                ...
                if result:
                    pixels[i, j] = c0 * alpha + c1 * beta + c2 * gamma
